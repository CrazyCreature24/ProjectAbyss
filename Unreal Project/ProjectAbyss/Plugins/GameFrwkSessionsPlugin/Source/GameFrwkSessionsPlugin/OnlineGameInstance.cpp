...
void UOnlineGameInstance::SetupConsoleCommands()
{
	HostConsoleCommand = IConsoleManager::Get().RegisterConsoleCommand(TEXT("Host"), TEXT("Host a Game"), FConsoleCommandWithArgsDelegate::CreateLambda(
		[&](const TArray< FString >& Args)
		{

			APlayerController* PController = UGameplayStatics::GetPlayerController(GetWorld(), 0);
			if (PController) {
				//FString command = TEXT("Travel " + Args[0] + "?listen"); // Type in map you want to travel too.
				FString command = TEXT("Travel Divebot_Testing?listen");
				PController->ConsoleCommand(command, true);
			}
		}
	), ECVF_Default);

	JoinConsoleCommand = IConsoleManager::Get().RegisterConsoleCommand(TEXT("Join"), TEXT("Join a Game"), FConsoleCommandWithArgsDelegate::CreateLambda(
		[&](const TArray< FString >& Args)
		{
			...
			if (PController) {
				...
				if (Args.Num() < 1)
				{
					command = TEXT("Travel 127.0.0.1");
				}
				else
				{
					command = TEXT("Travel " + Args[0]); // Type in an ip address to connect to other players.
				}
				//FString command = TEXT("Travel " + Args[0]); Type in an ip address to connect to other players.
				PController->ConsoleCommand(command, true);
			}
		}
	), ECVF_Default);
